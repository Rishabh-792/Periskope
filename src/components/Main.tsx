...
import { IoMdSend, IoMdTime, IoIosPeople } from 'react-icons/io'
...
import { FiRefreshCw, FiEdit3 } from "react-icons/fi";
import { RiListCheck2 } from "react-icons/ri";
import { BiSolidNetworkChart } from "react-icons/bi";

import { VscListSelection } from "react-icons/vsc";
import { RiListSettingsLine, RiFolderImageFill } from "react-icons/ri";
...
const sidebarItems = [
  { icon: <HiMiniArrowLeftStartOnRectangle  size={24} />, label: 'left' },
  { icon: <FiRefreshCw size={24} />, label: 'Refresh' },
  { icon: <FiEdit3 size={24} />, label: 'Edit' },
  { icon: <VscListSelection  size={24} />, label: 'Announcements' },
  { icon: <RiListCheck2  size={24} />, label: 'Users' },
  { icon: <BiSolidNetworkChart size={24} />, label: 'Library' },
  { icon: <IoIosPeople size={24} />, label: 'People' },
  { icon: <CiAt size={24} />, label: 'At' },
  { icon: <RiFolderImageFill size={24} />, label: 'Settings' },
  { icon: <RiListSettingsLine size={24} />, label: 'Settings' },
]
...
export default function Main({ selectedChat, userId }: Props) {
  ...
  return (
    <main className="flex bg-[#ece5dd] h-full w-full" style={{ backgroundImage: `url(${whatsappBg.src})` }}>
      ...
      {/* Right Icon Sidebar */}
        <aside className="w-16 flex flex-col items-center justify-between bg-white border-l border-gray-200 py-4">

          <div className="mb-6 h-8 w-8 rounded-full bg-transparent" />

          <div className="flex flex-col gap-4 flex-1 items-center text-gray-400">
            {sidebarItems.map((item, index) => (
              <button
                key={index}
                onClick={() => setActiveIndex(index)}
                className={`p-2 rounded-md hover:bg-gray-100 transition text-gray-400`}
                title={item.label}
              >
                {item.icon}
              </button>
